package net.arphex.world.features.configurations;
...
public record StructureFeatureConfiguration(
   ...
   public static final Codec<StructureFeatureConfiguration> CODEC = RecordCodecBuilder.create(
      builder -> builder.group(
               ...
               RegistryCodecs.homogeneousList(Registries.BLOCK).fieldOf("ignored_blocks").forGetter(config -> config.ignoredBlocks),
               Vec3i.offsetCodec(48).optionalFieldOf("offset", Vec3i.ZERO).forGetter(config -> config.offset)
            )
