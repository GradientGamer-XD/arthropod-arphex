...
public class LongLegsFlyEntity extends Monster implements GeoEntity {
   public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(LongLegsFlyEntity.class, EntityDataSerializers.BOOLEAN);
   public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(LongLegsFlyEntity.class, EntityDataSerializers.STRING);
   public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(LongLegsFlyEntity.class, EntityDataSerializers.STRING);
   public static final EntityDataAccessor<Integer> DATA_randomsize = SynchedEntityData.defineId(LongLegsFlyEntity.class, EntityDataSerializers.INT);
   ...
   public LongLegsFlyEntity(SpawnEntity packet, Level world) {
      this((EntityType<LongLegsFlyEntity>)ArphexModEntities.LONG_LEGS_FLY.get(), world);
   }

   public LongLegsFlyEntity(EntityType<LongLegsFlyEntity> type, Level world) {
      super(type, world);
      this.xpReward = 2;
      this.setNoAi(false);
   ...
   protected void defineSynchedData() {
      ...
      this.entityData.define(TEXTURE, "cranefly");
      this.entityData.define(DATA_randomsize, 0);
   }
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(4, new WaterAvoidingRandomStrollGoal(this, 1.5));
      this.goalSelector.addGoal(5, new FollowMobGoal(this, 1.0, 50.0F, 50.0F));
   ...
   public void baseTick() {
      super.baseTick();
      LongFlyTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
   ...
   public static void init() {
      SpawnPlacements.register(
         (EntityType)ArphexModEntities.LONG_LEGS_FLY.get(), Type.ON_GROUND, Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
            int x = pos.getX();
            int y = pos.getY();
            int z = pos.getZ();
   ...
   private PlayState movementPredicate(AnimationState event) {
      if (!this.animationprocedure.equals("empty")) {
         ...
         return event.setAndContinue(RawAnimation.begin().thenLoop("animation.cranefly.flying"));
      } else if (this.isDeadOrDying()) {
         return event.setAndContinue(RawAnimation.begin().thenPlay("animation.cranefly.death"));
      } else if (this.isInWaterOrBubble()) {
         return event.setAndContinue(RawAnimation.begin().thenLoop("animation.cranefly.idle"));
      } else if (this.isShiftKeyDown()) {
         return event.setAndContinue(RawAnimation.begin().thenLoop("animation.cranefly.idle"));
      } else {
         return !this.onGround()
            ? event.setAndContinue(RawAnimation.begin().thenLoop("animation.cranefly.flying"))
            : event.setAndContinue(RawAnimation.begin().thenLoop("animation.cranefly.idle"));
      }
   }
   ...
   private PlayState attackingPredicate(AnimationState event) {
      ...
      if (this.swinging && event.getController().getAnimationState() == State.STOPPED) {
         ...
         return event.setAndContinue(RawAnimation.begin().thenPlay("animation.cranefly.attack"));
      } else {
