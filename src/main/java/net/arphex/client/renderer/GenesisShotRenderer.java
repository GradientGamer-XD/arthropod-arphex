...
public class GenesisShotRenderer extends EntityRenderer<GenesisShotEntity> {
   private static final ResourceLocation texture = new ResourceLocation("arphex:textures/entities/genesis_shot.png");
   private final Modelgenesis_shot model;

   public GenesisShotRenderer(Context context) {
      super(context);
      this.model = new Modelgenesis_shot(context.bakeLayer(Modelgenesis_shot.LAYER_LOCATION));
   }

   public void render(GenesisShotEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(GenesisShotEntity entity) {
      return texture;
   }
}
