...
public class ChronoSpearShotRenderer extends EntityRenderer<ChronoSpearShotEntity> {
   ...
   public ChronoSpearShotRenderer(Context context) {
      super(context);
      this.model = new Modelgenesis_shot(context.bakeLayer(Modelgenesis_shot.LAYER_LOCATION));
   }

   public void render(ChronoSpearShotEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(ChronoSpearShotEntity entity) {
      return texture;
   }
}
