...
public class GravitonShotRenderer extends EntityRenderer<GravitonShotEntity> {
   private static final ResourceLocation texture = new ResourceLocation("arphex:textures/entities/black.png");
   private final Modelgenesis_shot model;

   public GravitonShotRenderer(Context context) {
      super(context);
      this.model = new Modelgenesis_shot(context.bakeLayer(Modelgenesis_shot.LAYER_LOCATION));
   }

   public void render(GravitonShotEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(GravitonShotEntity entity) {
      return texture;
   }
}
