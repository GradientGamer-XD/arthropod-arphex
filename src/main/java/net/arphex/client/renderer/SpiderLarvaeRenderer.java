...
public class SpiderLarvaeRenderer extends GeoEntityRenderer<SpiderLarvaeEntity> {
   ...
   public SpiderLarvaeRenderer(Context renderManager) {
      super(renderManager, new SpiderLarvaeModel());
      this.shadowRadius = 0.1F;
      this.addRenderLayer(new SpiderLarvaeLayer(this));
   ...
   public RenderType getRenderType(SpiderLarvaeEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
      return RenderType.entityTranslucent(this.getTextureLocation(animatable));
   }

   public void preRender(
      PoseStack poseStack,
      SpiderLarvaeEntity entity,
      BakedGeoModel model,
   ...
   public void applyRotations(SpiderLarvaeEntity entity, PoseStack poseStack, float ageInTicks, float netHeadYaw, float partialTick) {
   ...
   private boolean tryNyfsRotation(SpiderLarvaeEntity entity, PoseStack poseStack, float partialTick) {
      try {
         if (!iClimberEntityClass.isInstance(entity)) {
            ...
            if (orientationObj == null) {
               ...
               float inverseScale = 0.71428573F;
