...
public class ScorpioidShadowCloneSpawnProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
      if (entity != null) {
         ...
         if (Mth.nextInt(RandomSource.create(), 1, 10) == 5) {
            ArphexModVariables.MapVariables.get(world).clonesize = Mth.nextDouble(RandomSource.create(), 1.25, 30.0);
            ArphexModVariables.MapVariables.get(world).syncData(world);
         } else {
            ArphexModVariables.MapVariables.get(world).clonesize = Mth.nextDouble(RandomSource.create(), 1.25, 5.0);
            ArphexModVariables.MapVariables.get(world).syncData(world);
         }

         if (Mth.nextInt(RandomSource.create(), 1, 10) == 5) {
            if (Mth.nextInt(RandomSource.create(), 1, 5) == 2) {
               if (entity instanceof ScorpioidShadowCloneEntity animatable) {
                  animatable.setTexture("scorpioidbloodluster2");
               }
            } else if (entity instanceof ScorpioidShadowCloneEntity animatable) {
               animatable.setTexture("scorpioidchaser");
            }
         }
