...
public class SeismicCommandProcedure {
         ...
         if (foundplayers) {
            if (BoolArgumentType.getBool(arguments, "enabled")) {
               if (entity instanceof Player _player && !_player.level().isClientSide()) {
                  _player.displayClientMessage(Component.literal("§aSeismic power enabled for selected players"), false);
               }
            } else if (entity instanceof Player _player && !_player.level().isClientSide()) {
               _player.displayClientMessage(Component.literal("§cSeismic power disabled for selected players"), false);
            }
         }
      }
   }
