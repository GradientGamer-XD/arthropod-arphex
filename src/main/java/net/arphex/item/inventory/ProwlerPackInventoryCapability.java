...
public class ProwlerPackInventoryCapability implements ICapabilitySerializable<CompoundTag> {
   private final LazyOptional<ItemStackHandler> inventory = LazyOptional.of(this::createItemHandler);

   @SubscribeEvent
   @OnlyIn(Dist.CLIENT)
   public static void onItemDropped(ItemTossEvent event) {
      if (event.getEntity().getItem().getItem() == ArphexModItems.PROWLER_PACK.get() && Minecraft.getInstance().screen instanceof BackpackScreen) {
   ...
   private ItemStackHandler createItemHandler() {
      return new ItemStackHandler(27) {
         ...
         public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
            return stack.getItem() != ArphexModItems.PROWLER_PACK.get();
         }
